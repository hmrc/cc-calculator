...
class TFCCalculatorSpec extends PlaySpec with FakeCCCalculatorApplication {
  ...
  "TFCCalculatorService" must {
    ...
    "return a Future[AwardPeriod] result when household eligibility is true" in {
      ...
      val result = tfcCalculator.award(
        TFCCalculatorInput(from = fromDate, until = untilDate, householdEligibility = true, periods = List())
      )
    ...
    "return an empty Future[AwardPeriod] result when household eligibility is false" in {
      ...
      val result = await(
        tfcCalculator.award(
          TFCCalculatorInput(from = null, until = null, householdEligibility = false, periods = List())
        )
      )
    ...
    "return a calculated TFC for 1 TFC period with 1 child" in {
      ...
      val tfcPeriod = new models.input.tfc.TFCPeriod(
        from = fromDate,
        until = untilDate,
    ...
    "return a calculated TFC for 2 TFC periods" in {
      ...
      val tfcPeriodList = List(
        new models.input.tfc.TFCPeriod(
          from = fromDate,
          until = untilDate,
          periodEligibility = false,
          children = List()
    ...
    "return childcare cost for a period" in {
      ...
      val untilDate     = LocalDate.parse("2016-10-01T18:46:17", formatter)
      val child = TFCChild(
        childcareCost = BigDecimal(115.38),
    ...
    "return maximum topup  when child care monthly cost 700)" in {
      ...
      val untilDate     = LocalDate.parse("2017-10-01T18:46:17", formatter)
      val child = TFCChild(
        childcareCost = BigDecimal(700.00),
        qualifying = true,
        from = Some(fromDate),
        until = Some(untilDate),
        disability = TFCDisability(disabled = false, severelyDisabled = false)
    ...
    "return maximum topup  when child care monthly cost 833.34)" in {
      ...
      val untilDate     = LocalDate.parse("2017-10-01T18:46:17", formatter)
      val child = TFCChild(
        childcareCost = BigDecimal(833.34),
        qualifying = true,
        from = Some(fromDate),
        until = Some(untilDate),
        disability = TFCDisability(disabled = false, severelyDisabled = false)
    ...
    "return maximum topup  when child care monthly cost 1000)" in {
      ...
      val untilDate     = LocalDate.parse("2017-10-01T18:46:17", formatter)
      val child = TFCChild(
        childcareCost = BigDecimal(1000.00),
        qualifying = true,
        from = Some(fromDate),
        until = Some(untilDate),
        disability = TFCDisability(disabled = false, severelyDisabled = false)
    ...
    "return maximum topup  when child care monthly cost 1666.67, child disabled)" in {
      ...
      val untilDate     = LocalDate.parse("2016-10-01T18:46:17", formatter)
      val child = TFCChild(
        childcareCost = BigDecimal(1666.67),
        qualifying = true,
        from = Some(fromDate),
        until = Some(untilDate),
        disability = TFCDisability(disabled = true, severelyDisabled = false)
    ...
    "return maximum topUp  when child care monthly cost 5500, child disabled)" in {
      ...
      val untilDate     = LocalDate.parse("2017-10-01T18:46:17", formatter)
      val child = TFCChild(
        childcareCost = BigDecimal(1800.00),
        qualifying = true,
        from = Some(fromDate),
        until = Some(untilDate),
        disability = TFCDisability(disabled = true, severelyDisabled = false)
    ...
    "return calculated contribution for a child with monthly childcare cost 833 for current year" in {
      ...
      val untilDate     = LocalDate.parse("2016-10-01T18:46:17", formatter)
      val child = TFCChild(
        childcareCost = BigDecimal(833.33),
        qualifying = true,
        from = Some(fromDate),
        until = Some(untilDate),
        disability = TFCDisability(disabled = false, severelyDisabled = false)
    ...
    "return calculated contribution for a disabled child with monthly childcare cost 1666.66 for next year" in {
      ...
      val untilDate     = LocalDate.parse("2017-08-31T18:46:17", formatter)
      val child = TFCChild(
        childcareCost = BigDecimal(1666.66),
        qualifying = true,
        from = Some(fromDate),
        until = Some(untilDate),
        disability = TFCDisability(disabled = true, severelyDisabled = false)
    ...
    "return output children for 2 children where 1 child is qualifying" in {
      ...
      result shouldBe List(
        TFCOutputChild(
          childCareCost = BigDecimal(200.00),
          childContribution = TFCContribution(parent = 600.00, government = 0.00, totalChildCareSpend = 600.00)
        ),
        TFCOutputChild(
          childCareCost = BigDecimal(800.00),
          childContribution = TFCContribution(parent = 2400.00, government = 00.00, totalChildCareSpend = 2400.00)
        )
      )
    }
    ...
    "return output children for 2 children where both children are qualifying" in {
      ...
      result shouldBe List(
        ...
        TFCOutputChild(
          childCareCost = BigDecimal(800.00),
          childContribution = TFCContribution(parent = 2400.00, government = 0.00, totalChildCareSpend = 2400.00)
        )
      )
    }
    ...
    "Calculate the Total Period contributions for 2 children" in {
      val tfcCalculator = new TFCCalculator(tcfConfig) {}
      val child1 = TFCOutputChild(
        childCareCost = BigDecimal(200.00),
        childContribution = TFCContribution(BigDecimal(480.00), BigDecimal(120.00), BigDecimal(600.00))
      )
      val child2 = TFCOutputChild(
        childCareCost = BigDecimal(200.00),
        childContribution = TFCContribution(BigDecimal(480.00), BigDecimal(120.00), BigDecimal(600.00))
      )
    ...
    "Calculate the Total Period contributions for 3 children" in {
      ...
      val child3 = TFCOutputChild(
        childCareCost = BigDecimal(200.00),
        childContribution = TFCContribution(BigDecimal(20.00), BigDecimal(100.00), BigDecimal(100.00))
      )
