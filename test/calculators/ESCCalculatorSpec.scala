...
class ESCCalculatorSpec
    extends PlaySpec
    with FakeCCCalculatorApplication
    with MockitoSugar
    with org.scalatest.PrivateMethodTester
    with CCCalculatorHelper {
  ...
  "escCalcService" must {
    ...
    "(TY2016) get revised personal allowance when tax code is not provided (gross < 100000) " in {
      ...
      val result = escCalc.getPersonalAllowance(
        income,
        escConfig.getConfig(period.from, income.niCategory.toUpperCase.trim, location)
      )
    ...
    "allocate tax earnings to 20% rate band if tax code is BR" in {
      ...
      val outputModel = CalculationTaxBands(
        zeroRateBand = 0.00,
        basicRateBand = 10000.00,
        higherRateBand = 0.00,
        additionalRateBand = 0.00
      )
      val result = escCalcTax.allocateAmountToTaxBands(
        taxableEarnings,
        PA,
        calcPeriod,
        taxCode,
    ...
    "allocate tax earnings to 0% rate band if tax code is NT" in {
      ...
      val outputModel =
        CalculationTaxBands(zeroRateBand = 0.00, basicRateBand = 0.00, higherRateBand = 0.00, additionalRateBand = 0.00)
      val result = escCalcTax.allocateAmountToTaxBands(
        taxableEarnings,
        PA,
        calcPeriod,
    ...
    "allocate earnings to 0, basic, higher and additional rate bands (earnings > higher rate ceiling)" in {
      ...
      val outputModel = CalculationTaxBands(
        zeroRateBand = 11500,
        basicRateBand = 33500,
        higherRateBand = 105000,
        additionalRateBand = 0.01
      )
      val result = escCalcTax.allocateAmountToTaxBands(
        taxableEarnings,
        PA,
    ...
    "allocate earnings to 0, basic, higher and additional rate bands (earnings > higher rate ceiling, in scotland)" in {
      ...
      val result = escCalcTax.allocateAmountToTaxBands(
        ...
        Periods.Yearly,
        taxCode,
        escConfig.getConfig(period.from, "", locationScotland)
      )
    ...
    "determine exemption amount (pre 2011, relevant earnings = 0)" in {
      ...
      val toDate           = LocalDate.parse("21-05-2017", formatter)
      val period           = ESCPeriod(from = fromDate, until = toDate, claimants = List(), children = List())
      val claimant = models.input.esc.ESCClaimant(
        qualifying = true,
      ...
      val result = escCalcHelpers.determineMaximumIncomeRelief(
        pre2011,
        relevantEarnings,
        calcPeriod,
        taxCode,
    ...
    "determine tax amount for each band (Income is 50000)" in {
      ...
      val inputModel = CalculationTaxBands(
        zeroRateBand = 10600,
        basicRateBand = 31865,
        higherRateBand = 7535,
        additionalRateBand = 0.00
      ...
      val outputModel =
        CalculationTaxBands(zeroRateBand = 0.00, basicRateBand = 6373, higherRateBand = 3014, additionalRateBand = 0.00)
      val result = escCalcTax.calculateTaxPerBand(inputModel, escConfig.getConfig(period.from, "", location))
      result shouldBe outputModel
    }
    ...
    "determine tax amount for each band (Income is 150000)" in {
      ...
      val inputModel = CalculationTaxBands(
        zeroRateBand = 10600,
        basicRateBand = 31865,
        higherRateBand = 107535.00,
        additionalRateBand = 0.00
      )
      val outputModel = CalculationTaxBands(
        zeroRateBand = 0.00,
        basicRateBand = 6373,
        higherRateBand = 43014.00,
        additionalRateBand = 0.00
      )
      val result = escCalcTax.calculateTaxPerBand(inputModel, escConfig.getConfig(period.from, "", location))
      result shouldBe outputModel
    ...
    "Determine relevant earnings (income is 9000) if taxable income is less than personal allowance" in {
      ...
      val result =
        escCalcHelpers.getAnnualRelevantEarnings(income, escConfig.getConfig(period.from, "", location))
    ...
    "calculate savings per claimant (single claimant, post 2011, gross < personal allowance, voucher amount < max relief) (Monthly)" in {
      ...
      val inputClaimant = ESCClaimant(
        ...
        currentIncome = Some(ESCIncome(Some(9000.00), Some(0.00))),
        vouchers = true,
      ...
      val period = ESCPeriod(
        ...
        children = List(buildChild(childCareCost = 500))
      )
    ...
    "calculate savings per claimant (single claimant, post 2011, gross < basic rate limit, voucher amount > max relief) (Monthly)" in {
      ...
      val inputClaimant = ESCClaimant(
        ...
        currentIncome = Some(ESCIncome(Some(42700.00), Some(100.00))),
        vouchers = true,
        escStartDate = fromDate
      )
      val period = ESCPeriod(
      ...
      val outputClaimant = models.output.esc.ESCClaimant(
        qualifying = true,
        eligibleMonthsInTaxYear = 12,
        isPartner = false,
        income = models.output.esc.ESCIncome(taxablePay = 41500.00, gross = 42700.00, taxCode = "", niCategory = "A"),
        vouchers = true,
        escAmount = 243.00,
        escAmountPeriod = Periods.Monthly,
        escStartDate = fromDate,
        savings = ESCSavings(taxSaving = 48.60, niSaving = 29.16, totalSaving = 77.76),
        taxAndNIBeforeSacrifice = models.output.esc.ESCTaxAndNi(taxPaid = 500.00, niPaid = 345.36),
        taxAndNIAfterSacrifice = models.output.esc.ESCTaxAndNi(taxPaid = 451.40, niPaid = 316.20)
      )
    ...
    "Have correct total ESC Tax Savings for Scotland" in {
      ...
      val inputClaimant = ESCClaimant(
        ...
        isPartner = false,
        eligibleMonthsInPeriod = 12,
        currentIncome = Some(ESCIncome(Some(15000))),
        vouchers = true,
        escStartDate = fromDate
      )
      val period = ESCPeriod(
    ...
    "Have correct Tax Savings for Scotland with amount over to higher rate" in {
      ...
      val inputClaimant = ESCClaimant(
        ...
        isPartner = false,
        eligibleMonthsInPeriod = 12,
        currentIncome = Some(ESCIncome(Some(49000))),
        vouchers = true,
        escStartDate = fromDate
      )
      val period = ESCPeriod(
        ...
        until = toDate,
        claimants = List(inputClaimant),
        children = List(buildChild(childCareCost = 300))
      )
    ...
    "Have correct Tax Savings for Scotland with amount over to additional rate" in {
      ...
      val inputClaimant = ESCClaimant(
        ...
        isPartner = false,
        eligibleMonthsInPeriod = 12,
        currentIncome = Some(ESCIncome(Some(150012))),
        vouchers = true,
        escStartDate = fromDate
      )
      val period = ESCPeriod(
    ...
    "Have the correct total NI Savings for Scotland when paid over 46k threshold" when {
      "200 childcare costs" in {
        ...
        val inputClaimant = ESCClaimant(
          ...
          isPartner = false,
          eligibleMonthsInPeriod = 12,
          currentIncome = Some(ESCIncome(Some(47000))),
          vouchers = true,
          escStartDate = fromDate
        )
        val period = ESCPeriod(
      ...
      "100 childcare costs" in {
        ...
        val period = ESCPeriod(
          ...
          until = toDate,
          claimants = List(inputClaimant),
          children = List(buildChild(childCareCost = 100))
        )
    ...
    "Have the correct total NI Savings for Scotland when paid over 100000k" when {
      "with 300 childcare costs" in {
        ...
        val inputClaimant = ESCClaimant(
          ...
          isPartner = false,
          eligibleMonthsInPeriod = 12,
          currentIncome = Some(ESCIncome(Some(110000))),
          vouchers = true,
          escStartDate = fromDate
        )
        val period = ESCPeriod(
    ...
    "Have the correct NI exemption for Scotland when paid 45k" in {
      ...
      val inputClaimant = ESCClaimant(
        ...
        isPartner = false,
        eligibleMonthsInPeriod = 12,
        currentIncome = Some(ESCIncome(Some(45000))),
        vouchers = true,
        escStartDate = fromDate
      )
      val period = ESCPeriod(
    ...
    "calculate savings per claimant (single claimant, post 2011, gross < additional rate limit, voucher amount < max relief) (Monthly)" in {
      ...
      val inputClaimant = ESCClaimant(
        ...
        isPartner = false,
        eligibleMonthsInPeriod = 1,
        currentIncome = Some(ESCIncome(Some(50000.00))),
        vouchers = true,
        escStartDate = fromDate
      )
      val period = ESCPeriod(
        ...
        until = toDate,
        claimants = List(inputClaimant),
        children = List(buildChild(childCareCost = 20.77, childCareCostPeriod = Periods.Weekly))
      )
      ...
      val outputClaimant = models.output.esc.ESCClaimant(
        ...
        ESCTaxAndNi(766.60, 361.00),
        ESCTaxAndNi(730.60, 359.20)
      )
    ...
    "calculate savings per claimant (single claimant, post 2011, gross > additional rate limit, voucher amount > max relief) (Monthly)" in {
      ...
      val period = ESCPeriod(
        ...
        until = toDate,
        claimants = List(inputClaimant),
        children = List(buildChild(childCareCost = 115.38, childCareCostPeriod = Periods.Weekly))
      )
      ...
      val outputClaimant = models.output.esc.ESCClaimant(
        ...
        eligibleMonthsInTaxYear = 1,
        isPartner = false,
        income = models.output.esc.ESCIncome(taxablePay = 150001, gross = 150001, taxCode = "", niCategory = "A"),
        vouchers = true,
        escAmount = 110.00,
        escAmountPeriod = Periods.Monthly,
        escStartDate = fromDate,
    ...
    "calculate savings per claimant (two claimants, first - post 2011, gross > additional rate limit, voucher amount < max relief, second - post 2011, gross < additional rate limit, voucher amount < max relief) (Monthly)" in {
      ...
      val inputClaimant2 = ESCClaimant(
        qualifying = true,
        isPartner = true,
        eligibleMonthsInPeriod = 1,
        currentIncome = Some(ESCIncome(Some(150001))),
      ...
      val period = ESCPeriod(
        from = fromDate,
        until = toDate,
        claimants = List(inputClaimant, inputClaimant2),
        children = List(buildChild(childCareCost = 80), buildChild(childCareCost = 120))
      )
    ...
    "calculate tax and NI savings per claimant (single period, one claimant, 0 eligible months) (Monthly)" in {
      ...
      val outputClaimant = models.output.esc.ESCClaimant(
        ...
        escStartDate = fromDate,
        savings = ESCSavings(),
        taxAndNIBeforeSacrifice = models.output.esc.ESCTaxAndNi(),
        taxAndNIAfterSacrifice = models.output.esc.ESCTaxAndNi()
      )
    ...
    "calculate tax and NI savings per claimant (period 1 - 9 eligible months, period 2 - 2 eligible months, one claimant) (taxablePay 50000.00) (Monthly)" in {
      ...
      val period = ESCPeriod(
        ...
        until = toDate,
        claimants = List(inputClaimant),
        children = List(buildChild(childCareCost = 90))
      )
      val period2 = ESCPeriod(
        from = fromDate2,
        until = toDate2,
        claimants = List(inputClaimant2),
        children = List(buildChild(childCareCost = 90))
      )
      ...
      val outputClaimant = models.output.esc.ESCClaimant(
        ...
        taxAndNIBeforeSacrifice = models.output.esc.ESCTaxAndNi(taxPaid = 8349.80, niPaid = 4003.08),
        taxAndNIAfterSacrifice = models.output.esc.ESCTaxAndNi(taxPaid = 7953.80, niPaid = 3983.28)
      )

      val outputTY = models.output.esc.ESCTaxYear(
        fromDate,
        toDate,
        ESCSavings(taxSaving = 396, niSaving = 19.8, totalSaving = 415.8),
        List(outputClaimant)
      )
      val resultTY = escCalc.getCalculatedTaxYears(ESCCalculatorInput(List(taxYear), location))
    ...
    "calculate tax savings per claimant (period 1 - 2 eligible months, period 2 - 0 eligible months, one claimant) (taxablePay 50000.00) (Monthly)" in {
      ...
      val outputTY = models.output.esc.ESCTaxYear(
        fromDate,
        toDate,
        ESCSavings(totalSaving = 75.6, taxSaving = 72, niSaving = 3.6),
        List(outputClaimant)
      )
      val resultTY = escCalc.getCalculatedTaxYears(ESCCalculatorInput(List(taxYear), location))
    ...
    "calculate tax savings per claimant (claimant 1 = period 1 - 10 eligible months, period 2 - 1 eligible months) (claimant 2 = period 1 - 1 eligible months," +
      " period 2 - 1 eligible months) (taxablePay both 50000.00) (Monthly)" in {
        ...
        val inputClaimant = ESCClaimant(
          ...
          currentIncome = Some(ESCIncome(Some(50000))),
          vouchers = true,
          escStartDate = fromDate
        )
        val inputPartner = ESCClaimant(
          qualifying = true,
          isPartner = true,
          eligibleMonthsInPeriod = 1,
          currentIncome = Some(ESCIncome(Some(50000))),
        ...
        val period = ESCPeriod(
          from = fromDate,
          until = toDate,
          claimants = List(inputClaimant, inputPartner),
          children = List(buildChild(childCareCost = 180))
        )
        val period2 = ESCPeriod(
          from = fromDate2,
          until = toDate2,
          claimants = List(inputClaimant2, inputPartner2),
          children = List(buildChild(childCareCost = 180))
        )
        ...
        val outputClaimant = models.output.esc.ESCClaimant(
          ...
          taxAndNIBeforeSacrifice = models.output.esc.ESCTaxAndNi(taxPaid = 7666.00, niPaid = 3610.00),
          taxAndNIAfterSacrifice = models.output.esc.ESCTaxAndNi(taxPaid = 7442.00, niPaid = 3598.8)
        )

        val outputPartner = models.output.esc.ESCClaimant(
          qualifying = true,
          eligibleMonthsInTaxYear = 2,
          isPartner = true,
          income = models.output.esc.ESCIncome(taxablePay = 50000.00, gross = 50000.00, taxCode = "", niCategory = "A"),
          ...
          taxAndNIAfterSacrifice = models.output.esc.ESCTaxAndNi(taxPaid = 717.00, niPaid = 358.52)
        )

        val outputTY = models.output.esc.ESCTaxYear(
          fromDate,
          toDate,
          ESCSavings(totalSaving = 287.28, taxSaving = 273.6, niSaving = 13.68),
          List(outputClaimant, outputPartner)
        )
        val resultTY = escCalc.getCalculatedTaxYears(ESCCalculatorInput(List(taxYear), location))
    ...
    "calculate tax savings per claimant (claimant 1 = period 1 - 0 eligible months, period 2 - 0 eligible months) (claimant 2 = period 1 - 0 eligible months," +
      " period 2 - 1 eligible months) (taxablePay both 50000.00) (Monthly)" in {
        ...
        val outputClaimant = models.output.esc.ESCClaimant(
          ...
          taxAndNIBeforeSacrifice = models.output.esc.ESCTaxAndNi(taxPaid = 0, niPaid = 0),
          taxAndNIAfterSacrifice = models.output.esc.ESCTaxAndNi(taxPaid = 0, niPaid = 0)
        )
        ...
        val outputTY = models.output.esc.ESCTaxYear(
          fromDate,
          toDate,
          ESCSavings(totalSaving = 37.8, taxSaving = 36, niSaving = 1.8),
          List(outputClaimant, outputPartner)
        )
        val resultTY = escCalc.getCalculatedTaxYears(ESCCalculatorInput(List(taxYear), location))
    ...
    "allocate gross earnings to NI bands (earnings in lower earnings level, TY2016, category A)" in {
      ...
      val outputModel = CalculationNIBands(
        lowerEarningsBand = 485,
        primaryEarningsBand = 0,
        upperEarningsBand = 0,
        aboveUpperEarningsBand = 0
      )
      val result = escCalcNi.allocateAmountToNIBands(grossPay, escConfig.getConfig(period.from, "A", location))
    ...
    "allocate gross earnings to NI bands (earnings in primary earnings level, TY2016, category A)" in {
      ...
      val outputModel = CalculationNIBands(
        lowerEarningsBand = 485.00,
        primaryEarningsBand = 0.01,
        upperEarningsBand = 0,
        aboveUpperEarningsBand = 0
      )
      val result = escCalcNi.allocateAmountToNIBands(grossPay, escConfig.getConfig(period.from, "A", location))
    ...
    "allocate gross earnings to NI bands (earnings in primary earnings level, TY2016, category B)" in {
      ...
      val outputModel = CalculationNIBands(
        lowerEarningsBand = 485.00,
        primaryEarningsBand = 187,
        upperEarningsBand = 0,
        aboveUpperEarningsBand = 0
      )
      val result = escCalcNi.allocateAmountToNIBands(grossPay, escConfig.getConfig(period.from, "B", location))
    ...
    "allocate gross earnings to NI bands (earnings in upper accrual earnings level, TY2016, category C)" in {
      ...
      val outputModel = CalculationNIBands(
        lowerEarningsBand = 485.00,
        primaryEarningsBand = 187,
        upperEarningsBand = 2665,
        aboveUpperEarningsBand = 0
      )
      val result = escCalcNi.allocateAmountToNIBands(grossPay, escConfig.getConfig(period.from, "C", location))
    ...
    "allocate gross earnings to NI bands (earnings in above upper earnings level, TY2016, category C)" in {
      ...
      val outputModel = CalculationNIBands(
        lowerEarningsBand = 485.00,
        primaryEarningsBand = 187,
        upperEarningsBand = 2860.01,
        aboveUpperEarningsBand = 0.0
      )
      val result = escCalcNi.allocateAmountToNIBands(grossPay, escConfig.getConfig(period.from, "C", location))
    ...
    "allocate gross earnings to NI bands (earnings in above upper earnings level, TY2017, category B)" in {
      ...
      val outputModel = CalculationNIBands(
        lowerEarningsBand = 490.00,
        primaryEarningsBand = 190,
        upperEarningsBand = 3073,
        aboveUpperEarningsBand = 6247
      )
      val result = escCalcNi.allocateAmountToNIBands(grossPay, escConfig.getConfig(period.from, "B", location))
    ...
    "determine NI amount for each band (earnings is 485 (lower earnings level), TY2016, category A)" in {
      ...
      val inputModel = CalculationNIBands(
        lowerEarningsBand = 485,
        primaryEarningsBand = 0,
        upperEarningsBand = 0,
        aboveUpperEarningsBand = 0
      ...
      val result = escCalcNi.calculateNIPerBand(inputModel, escConfig.getConfig(period.from, "A", location))
    ...
    "determine NI amount for each band (earnings is 672 (primary earnings level), TY2016, category B)" in {
      ...
      val result = escCalcNi.calculateNIPerBand(inputModel, escConfig.getConfig(period.from, "B", location))
    ...
    "determine NI amount for each band (earnings is 800 (upper accrual earnings level), TY2017, category A)" in {
      ...
      val inputModel = CalculationNIBands(
        lowerEarningsBand = 485.00,
        primaryEarningsBand = 187,
        upperEarningsBand = 128,
        aboveUpperEarningsBand = 0
      )
      val outputModel = CalculationNIBands(
        lowerEarningsBand = 0,
        primaryEarningsBand = 0,
        upperEarningsBand = 15.36,
        aboveUpperEarningsBand = 0
      )
      val result = escCalcNi.calculateNIPerBand(inputModel, escConfig.getConfig(period.from, "A", location))
    ...
    "determine NI amount for each band (earnings is 800 (upper accrual earnings level), TY2016, category C)" in {
      ...
      val result = escCalcNi.calculateNIPerBand(inputModel, escConfig.getConfig(period.from, "C", location))
    ...
    "determine NI amount for each band (earnings is 3500 (upper earnings level), TY2016, category A)" in {
      ...
      val inputModel = CalculationNIBands(
        lowerEarningsBand = 485.00,
        primaryEarningsBand = 187,
        upperEarningsBand = 2828,
        aboveUpperEarningsBand = 0
      )
      val outputModel = CalculationNIBands(
        lowerEarningsBand = 0,
        primaryEarningsBand = 0,
        upperEarningsBand = 339.36,
        aboveUpperEarningsBand = 0
      )
      val result = escCalcNi.calculateNIPerBand(inputModel, escConfig.getConfig(period.from, "A", location))
    ...
    "determine NI amount for each band (earnings is 4000 (above upper earnings level), TY2016, category A)" in {
      ...
      val inputModel = CalculationNIBands(
        lowerEarningsBand = 485.00,
        primaryEarningsBand = 187,
        upperEarningsBand = 2680,
        aboveUpperEarningsBand = 468
      )
      val outputModel = CalculationNIBands(
        lowerEarningsBand = 0,
        primaryEarningsBand = 0,
        upperEarningsBand = 321.60,
        aboveUpperEarningsBand = 9.36
      )
      val result = escCalcNi.calculateNIPerBand(inputModel, escConfig.getConfig(period.from, "A", location))
    ...
    "determine total NI due (Income 4000 monthly)" in {
      val inputModel = CalculationNIBands(
        lowerEarningsBand = 0,
        primaryEarningsBand = 319.8,
        upperEarningsBand = 23.4,
        aboveUpperEarningsBand = 9.36
      )
      val calcPeriod = Periods.Yearly
    ...
    "determine total NI due (Income 3500 monthly)" in {
      val inputModel = CalculationNIBands(
        lowerEarningsBand = 0,
        primaryEarningsBand = 155.9025,
        upperEarningsBand = 9.5355,
        aboveUpperEarningsBand = 0
      )
      val calcPeriod = Periods.Yearly
    ...
    "determine total NI savings per month after salary sacrifice for niCategory A (income 4000, voucher amount 200)" in {
      ...
      val result = escCalcNi.calculateNISavings(
        grossPay,
        reliefAmount,
        escConfig.getConfig(period.from, "A", location)
      )
    ...
    "determine total NI savings per month after salary sacrifice for niCategory B (income 4000, voucher amount 200)" in {
      ...
      val result = escCalcNi.calculateNISavings(
        grossPay,
        reliefAmount,
        escConfig.getConfig(period.from, "B", location)
      )
      result._1 shouldBe BigDecimal(2.48)
    ...
    "Generate total award with claimants (Total Award test)" in {
      ...
      val input: ESCCalculatorInput =
        json.validate[ESCCalculatorInput] match {
          case JsSuccess(value, _) =>
            value

          case JsError(errors) =>
            fail(s"Failed to validate input JSON: $errors")
        }

      val result: ESCCalculatorOutput = escCalc.award(input)
    ...
    "Return error for invalid tax code (Y)" in {
      ...
      try {
        val result = escCalcHelpers.getTaxCode(
          income,
          escConfig.getConfig(period.from, income.niCategory.toUpperCase.trim, location)
        )
        result shouldBe a[NoSuchElementException]
