...
class ESCCalculatorHelpers @Inject() (escConfig: ESCConfig)
  ...
  def validateCode(code: String): Boolean =
    code.equals("BR") || code.equals("D0") || code.equals("D1") || code.equals("NT") ||
      code.equals("0T") || code.startsWith("K")

  def validateCodeBasedOnEndsWith(code: String): Boolean =
    code.endsWith("L") || code.endsWith("M") || code.endsWith("N") ||
  ...
  def validateTaxCode(income: ESCTotalIncome): (BigDecimal, String) =
  ...
  def getTaxCode(income: ESCTotalIncome, config: ESCTaxYearConfig): String =
    income.taxCode.trim match {
      case code if code.trim.isEmpty => config.defaultTaxCode
      case _                         => validateTaxCode(income)._2
    }

  def getAnnualRelevantEarnings(
      income: ESCTotalIncome,
  ...
  private def determineMaximumIncomeReliefPost2011BasedOnRelevantEarnings(
  ...
  private def determineMaximumIncomeReliefPre2011(
      relevantEarnings: BigDecimal,
      calcPeriod: Periods.Period,
      taxCode: String
  ) =
  ...
  def subtractActualReliefFromIncome(
      income: BigDecimal,
  ...
  def populateClaimantModel(
    ...
    models.output.esc.ESCClaimant(
      ...
      isPartner = isPartner,
      income = models.output.esc
        .ESCIncome(taxablePay = taxablePay, gross = grossPay, taxCode = taxCode, niCategory = niCategory),
      vouchers = vouchers,
      escAmount = escAmount,
      ...
      taxAndNIBeforeSacrifice =
        models.output.esc.ESCTaxAndNi(taxPaid = taxPaidPreSacrifice, niPaid = niPaidPreSacrifice),
      taxAndNIAfterSacrifice =
        models.output.esc.ESCTaxAndNi(taxPaid = taxPaidPostSacrifice, niPaid = niPaidPostSacrifice)
    )

}

//Scottish tax changes
class ESCCalculatorTax @Inject() (escHelpers: ESCCalculatorHelpers) {

  private def allocateAmountToTaxBandsBasedOnTaxablePay(
      taxablePay: BigDecimal,
  ...
  private def allocateAmountToTaxBandsForAllButScotland(
      taxablePay: BigDecimal,
      personalAllowancePerPeriod: BigDecimal,
      calcPeriod: Periods.Period,
      config: ESCTaxYearConfig
    ...
    val higherRateCeilingPerPeriod: BigDecimal =
      escHelpers.roundToPound(escHelpers.annualAmountToPeriod(config.taxHigherBandUpperLimit, calcPeriod))
    val basicRateCapacityPerPeriod: BigDecimal =
      escHelpers.roundToPound(escHelpers.annualAmountToPeriod(config.taxBasicBandCapacity, calcPeriod))
    ...
    if (taxablePay <= personalAllowancePerPeriod) {
      ...
      CalculationTaxBands(
        ...
        basicRateBand = taxablePay - personalAllowancePerPeriod
      )
  ...
  private def allocateAmountToTaxBandsForScotland(
      taxablePay: BigDecimal,
      personalAllowancePerPeriod: BigDecimal,
      calcPeriod: Periods.Period,
      config: ESCTaxYearConfig
    ...
    val starterRateCapacityPerPeriod: BigDecimal =
      escHelpers.roundToPound(escHelpers.annualAmountToPeriod(config.taxStarterBandCapacity, calcPeriod))
    ...
    val intermediateCapacityPerPeriod: BigDecimal =
      escHelpers.roundToPound(escHelpers.annualAmountToPeriod(config.taxIntermediateBandCapacity, calcPeriod))
    val intermediateRateCeiling: BigDecimal = intermediateCapacityPerPeriod + basicRateCeiling
    val higherRateCapacityPerPeriod: BigDecimal =
      escHelpers.roundToPound(escHelpers.annualAmountToPeriod(config.taxHigherRateBandCapacity, calcPeriod))
  ...
  def totalTaxDue(taxAmountPerBand: CalculationTaxBands, calcPeriod: Periods.Period): BigDecimal = {
    ...
    escHelpers.round(escHelpers.annualAmountToPeriod(annualTaxAmount, calcPeriod))
  }

  def calculateTaxSavings(
      taxablePay: BigDecimal,
    ...
    val taxPerBandBeforeSacrifice: CalculationTaxBands =
      calculateTaxPerBand(
        allocateAmountToTaxBands(taxablePay, personalAllowance, calcPeriod, taxCode, config),
        config
      )

    val totalTaxDueBeforeSacrifice: BigDecimal = totalTaxDue(taxPerBandBeforeSacrifice, Periods.Yearly)
    val postSalarySacrificeEarnings: BigDecimal =
      escHelpers.subtractActualReliefFromIncome(taxablePay, reliefAmount, Periods.Yearly)

    val taxPerBandAfterSacrifice: CalculationTaxBands =
      calculateTaxPerBand(
        allocateAmountToTaxBands(postSalarySacrificeEarnings, personalAllowance, calcPeriod, taxCode, config),
        config
      )
    ...
    val taxSavingAmountPerMonth =
      escHelpers.determineTotalSavings(totalTaxDueBeforeSacrifice, totalTaxDueAfterSacrifice)
    (taxSavingAmountPerMonth, totalTaxDueBeforeSacrifice, totalTaxDueAfterSacrifice)
  }

  def calculateTaxPerBand(
      taxableAmountPerBand: CalculationTaxBands,
      config: ESCTaxYearConfig
  ): CalculationTaxBands = {
...
class ESCCalculatorNi @Inject() (val escHelpers: ESCCalculatorHelpers) {

  def allocateAmountToNIBands(grossPay: BigDecimal, config: ESCTaxYearConfig): CalculationNIBands = {
  ...
  def calculateNIPerBand(
      grossAmountPerBand: CalculationNIBands,
      config: ESCTaxYearConfig
  ): CalculationNIBands = {
  ...
  def totalNIDue(niAmountPerBand: CalculationNIBands, calcPeriod: Periods.Period): BigDecimal = {
    ...
    escHelpers.round(escHelpers.annualAmountToPeriod(annualNIAmount, calcPeriod))
  }
  ...
  def calculateNISavings(
    ...
    val niPerBandBeforeSacrifice: CalculationNIBands =
      calculateNIPerBand(allocateAmountToNIBands(grossPay, config), config)
    val totalNIDueBeforeSacrifice: BigDecimal = totalNIDue(niPerBandBeforeSacrifice, Periods.Yearly)
    val postSalarySacrificeEarnings: BigDecimal =
      escHelpers.subtractActualReliefFromIncome(grossPay, reliefAmount, Periods.Yearly)
    val niPerBandAfterSacrifice: CalculationNIBands =
      calculateNIPerBand(allocateAmountToNIBands(postSalarySacrificeEarnings, config), config)
    val totalNIDueAfterSacrifice: BigDecimal = totalNIDue(niPerBandAfterSacrifice, Periods.Yearly)
    // Total tax savings per one month
    val niSavingAmountPerMonth = escHelpers.determineTotalSavings(totalNIDueBeforeSacrifice, totalNIDueAfterSacrifice)
...
class ESCCalculator @Inject() (escConfig: ESCConfig, escCalc: ESCCalculatorTax, escCalcNI: ESCCalculatorNi)(
  ...
  /** @param maximumReliefAmount
    ...
    * @param relevantEarnings
    *   earnings from which user can buy vouchers
    * @param escAmountForPeriod
    *   real amount that we want to assign to user
    * @return
    *   actual amount of vouchers that user can buy
    */

  def getActualRelief(
      maximumReliefAmount: BigDecimal,
      relevantEarnings: BigDecimal,
      escAmountForPeriod: BigDecimal
  ): BigDecimal = {
  ...
  def determineSavingsPerClaimant(period: ESCPeriod, location: String): List[models.output.esc.ESCClaimant] = {
    ...
    val escAmountForPeriod: BigDecimal = escCalcNI.escHelpers.round(
      period.children
        .filter(_.qualifying)
        .foldLeft(BigDecimal(0.00))((acc, child) =>
          acc + escCalcNI.escHelpers.amountToMonthlyAmount(child.childCareCost, child.childCareCostPeriod)
        )
    )
    ...
    val (parentESCTaxAmount, partnerESCTaxAmount, parentESCNIAmount, partnerESCNIAmount)
      ...
      (periodParent, periodPartner) match {
        ...
        case (Some(parent), Some(partner)) if parent.qualifying && partner.qualifying =>
          ...
          val (maximumReliefAmountHE, relevantEarningsForTaxHE, relevantEarningsForNIHE) =
            getRelevantEarnings(
              period,
              claimantsByIncome.last.income,
              claimantsByIncome.last.isESCStartDateBefore2011,
              location
            )
          val (maximumReliefAmountLE, relevantEarningsForTaxLE, relevantEarningsForNILE) =
            getRelevantEarnings(
              period,
              claimantsByIncome.head.income,
              claimantsByIncome.head.isESCStartDateBefore2011,
              location
            )
          val reliefFromTaxHE = getActualRelief(maximumReliefAmountHE, relevantEarningsForTaxHE, escAmountForPeriod)
          val reliefFromTaxLE =
            getActualRelief(maximumReliefAmountLE, relevantEarningsForTaxLE, escAmountForPeriod - reliefFromTaxHE)
          val reliefFromNIHE =
            getActualRelief(
              maximumReliefAmountHE - reliefFromTaxHE,
              relevantEarningsForNIHE,
              escAmountForPeriod - reliefFromTaxHE - reliefFromTaxLE
            )
          val reliefFromNILE =
            getActualRelief(
              maximumReliefAmountLE - reliefFromTaxLE,
              relevantEarningsForNILE,
              escAmountForPeriod - reliefFromTaxHE - reliefFromTaxLE - reliefFromNIHE
            )
    ...
    for (claimant <- period.claimants) yield {
      ...
      val taxCode    = escCalcNI.escHelpers.getTaxCode(claimant.income, config)
      val personalAllowanceAmount: BigDecimal = getPersonalAllowance(claimant.income, config)
      ...
      val taxablePayMonthly: BigDecimal = escCalcNI.escHelpers.roundToPound(
        escCalcNI.escHelpers.annualAmountToPeriod(claimant.income.taxablePay, calcPeriod)
      )
      val grossPayMonthly: BigDecimal =
        escCalcNI.escHelpers.roundToPound(escCalcNI.escHelpers.annualAmountToPeriod(claimant.income.gross, calcPeriod))
      val personalAllowanceMonthly: BigDecimal = escCalcNI.escHelpers.roundToPound(
        escCalcNI.escHelpers.annualAmountToPeriod(personalAllowanceAmount, calcPeriod)
      )

      val taxSavingAmounts: (BigDecimal, BigDecimal, BigDecimal) =
        escCalc.calculateTaxSavings(
          taxablePayMonthly,
      ...
      val niSavingAmounts: (BigDecimal, BigDecimal, BigDecimal) =
        escCalcNI.calculateNISavings(grossPayMonthly, actualNIReliefAmount, config)

      escCalcNI.escHelpers.populateClaimantModel(
        claimant.qualifying,
  ...
  def getPersonalAllowance(income: ESCTotalIncome, config: ESCTaxYearConfig): BigDecimal =
    income.taxCode.trim match {
      case code if code.isEmpty => income.adjustPersonalAllowance(config.defaultPersonalAllowance)
      case _                    => escCalcNI.escHelpers.validateTaxCode(income)._1
    }

  private def determineRelief(
      period: ESCPeriod,
      location: String,
      claimant: ESCClaimant,
      escAmountForPeriod: BigDecimal
  ) = {
    ...
    val reliefFromNI =
      getActualRelief(maximumReliefAmount - reliefFromTax, relevantEarningsForNI, escAmountForPeriod - reliefFromTax)
    (reliefFromTax, reliefFromNI)
  }

  private def determineCalculatedListOfClaimantsPairs(
  ...
  /** @return
    ...
    */
  protected def getRelevantEarnings(
      period: ESCPeriod,
      income: ESCTotalIncome,
      isESCStartDateBefore2011: Boolean,
      location: String
  ) = {
    val config  = escConfig.getConfig(period.from, income.niCategory.toUpperCase.trim, location)
    val taxCode = escCalcNI.escHelpers.getTaxCode(income, config)
    ...
    val (relevantEarningsForTax, relevantEarningsForNI): (BigDecimal, BigDecimal) =
      escCalcNI.escHelpers.getAnnualRelevantEarnings(income, config)
    // maximum limit of £243 a month
    val maximumReliefAmount: BigDecimal = escCalcNI.escHelpers.determineMaximumIncomeRelief(
    ...
    (
      maximumReliefAmount,
      escCalcNI.escHelpers.annualAmountToPeriod(relevantEarningsForTax, Periods.Monthly),
      escCalcNI.escHelpers.annualAmountToPeriod(relevantEarningsForNI, Periods.Monthly)
    )
  }
  ...
  private def determineClaimantsForTaxYear(
      listOfClaimants: List[models.output.esc.ESCClaimant]
  ): List[models.output.esc.ESCClaimant] = {
