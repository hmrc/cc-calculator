...
//      })
//
//      // copy the original calculation swapping out the tax year that has been pro-rated and adjusting the total calculation value
//      award.copy(
      ...
      Future {
        request.getTaxCreditsEligibility match {
          case Success(result) =>
            ...
            AwardPeriod(
              tc = Some(createTCCalculation(calculatedTaxYears, annualIncome(calculatedTaxYears, incomeAdvice), incomeAdvice))
            )
          case Failure(e) =>
