...
trait ESCConfig extends CCConfig with ServicesConfig with MessagesObject with LoadConfig {
  ...
  private def getTaxYear(niCategoryCode: String, config: Configuration, location: String): ESCTaxYearConfig = {
    ...
    ESCTaxYearConfig(
      ...
      taxStarterBandCapacity = localConfig.getDouble("basic.band-capacity-annual-amount").getOrElse(0),
      ...
      taxIntermediateBandCapacity = localConfig.getDouble("basic.band-capacity-annual-amount").getOrElse(0),
