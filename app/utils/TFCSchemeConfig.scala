...
trait TFCConfig {
  lazy val maxNoOfChildren = configuration.getInt(s"tfc.max-no-of-children").getOrElse(25)
  lazy val maxNameLength = configuration.getInt(s"tfc.max-name-length").getOrElse(25)
